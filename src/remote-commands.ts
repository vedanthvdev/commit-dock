...
export function registerRemoteCommands(context: vscode.ExtensionContext): void {
  const channel = vscode.window.createOutputChannel('Commit Dock: Remotes');
  context.subscriptions.push(channel);

  context.subscriptions.push(
    vscode.commands.registerCommand('commitDock.showGitRemotes', async () => {
      ...
      try {
        const { stdout } = await execFileAsync(
          'git',
          ['-c', 'core.pager=cat', 'remote', '-v'],
          { cwd: root, maxBuffer: 2_000_000, ...gitExecFileBase },
        );
        channel.clear();
        channel.appendLine(stdout.trimEnd() || '(no remotes configured)');
        channel.show(true);
      } catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
        void vscode.window.showErrorMessage(`Commit Dock: could not read remotes — ${msg}`);
      }
    }),
  );
}
